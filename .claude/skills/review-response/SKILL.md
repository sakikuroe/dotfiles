...
- [reply_inline.sh](./scripts/reply_inline.sh): `bash .claude/skills/review-response/scripts/reply_inline.sh <PR番号> <comment_id> <body_file> <commit_hash|->` で, インライン review comment に返答する. 本文末尾に署名を自動付加する. コミットハッシュを指定するとそのコミットの URL を挿入し, `-` を渡すと挿入をスキップする.
