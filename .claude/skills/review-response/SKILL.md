...
- [reply_inline.sh](./scripts/reply_inline.sh): インライン review comment に返答する. コミットハッシュを指定するとそのコミットの URL を返答に挿入する. ハッシュの代わりに `-` を渡すと挿入をスキップする.
- [set_ready.sh](./scripts/set_ready.sh): draft PR を ready for review に切り替える.
- [add_progress_comment.sh](./scripts/add_progress_comment.sh): Issue に進捗コメントを追加投稿する.
