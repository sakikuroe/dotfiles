...
- `bash .claude/skills/review-response/scripts/fetch_reviews.sh <PR番号>` で PR の状態 / 全体レビュー / インライン review comment を 1 回で取得する。
    - 出力フィールド:
        - `state`、`isDraft`、`reviewDecision`、`mergeable`、`mergeStateStatus`: PR の状態判定に使う
        - `checks`: CI ステータス
        - `reviews`: 全体レビューの一覧であり、`body` に具体的な指摘や要求があれば修正対応が必要
        - `inline_comments`: インライン comment の一覧であり、`in_reply_to_id` が null のものが起点コメント、値が入っているものは既存スレッドへの返信である。未対応の起点コメントがあれば修正対応が必要
- 状態に応じて判定結果を記録する。
    - `MERGED` の場合 → マージ済み
