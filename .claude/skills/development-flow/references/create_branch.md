...
- worktree を用意する。
    ...
    - `bash .claude/skills/development-flow/scripts/create_worktree.sh <branch-name>` で作成・再利用を一括して行う。
        - 実行場所: メインリポジトリーから実行するのが基本である。worktree 内からでも動作する (スクリプトが `git worktree list` の先頭行でメインリポジトリーを特定する)。
        - コマンド例:
            ```bash
            # 実行場所: メインリポジトリー
            ...
            ```
- ブランチ名を進捗コメントで記録する。
    - `bash .claude/skills/development-flow/scripts/add_progress_comment.sh <issue番号> <body_file>` で投稿する。
