...
実装方針の承認を得た Issue に対して、Issue に紐付く短寿命の作業ブランチと、開発用の worktree を用意する。既存作業を再開する場合は、新しく作らずに既存のブランチと worktree を再利用する。同じ Issue に対してブランチや worktree が乱立すると、どれが正かを判断できなくなるためである。
