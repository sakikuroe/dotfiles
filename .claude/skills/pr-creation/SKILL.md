---
name: pr-creation
description: push と PR 作成を担うスキルである。作業ブランチを push し、default branch 向けの PR を作成するときに使用する。単独で PR を作成する場面でも使用する。
---
...
- [create_pr.sh](./scripts/create_pr.sh): `bash .claude/skills/pr-creation/scripts/create_pr.sh <タイトル> <body_file> <head_branch>` で、本文末尾に署名を自動付加して PR を作成する。
...
- [set_ready.sh](./scripts/set_ready.sh): `bash .claude/skills/pr-creation/scripts/set_ready.sh <PR番号>` で draft PR を ready に切り替える。
