...
-- ここでは Nushell を既定にし、`exec nu -l` のようなコマンド文字列が画面に残る実装を避ける。
...
-- 新規タブ作成時のシェル候補を `shell_order` に従って生成する。
...
-- 初期タブは `config.default_prog` (Nushell) で起動し、Bash を選択した場合は Bash タブを作成して入れ替える。
wezterm.on('gui-startup', function(cmd)
  -- `wezterm start --cwd .` のように起動すると、`cmd` には `cwd` などの情報だけが入り、`args` が空のことがある。
  -- その状態で `spawn_window(cmd)` すると、起動するシェルが環境依存になり、Bash で起動したあとに
  -- `exec nu -l` で切り替えたような表示が残るケースがある。
  -- ここでは、`args` が空のときは `default_prog` と同じ `nu -l` を明示し、起動時の挙動を固定する。
...
-- ペインの視認性を設定する。
-- ペイン境界線 (分割線) の色を設定する。
-- 色は `#RRGGBB` 形式で指定し、ここでは白 (#ffffff) にする。
...
-- タブバーの表示と操作を設定する。
-- 装飾付きタブバーの有効 / 無効を設定する。
-- false の場合は、余計な装飾を減らしたシンプルな表示になる。
config.use_fancy_tab_bar = false
-- タブ 1 つあたりの最大表示幅を設定する。
-- 単位は文字セルで、タブ名が長い場合はこの幅で省略される。
config.tab_max_width = 100
-- マウスホイールによるタブ切り替えの有効 / 無効を設定する。
-- true の場合はタブ切り替えが優先され、false の場合はスクロールが優先される。
config.mouse_wheel_scrolls_tabs = false

-- 通知の表示を設定する。
-- `notification_handling` は、端末アプリケーションが通知 (デスクトップ通知など) を要求した場合の扱いを設定する。
-- ここでは通知を表示しないことで、意図しないポップアップを防ぐ。
...
config.keys = {
  ...
  -- `skip_close_confirmation_for_processes_named` に該当すると、確認表示が省略される場合がある。
  {
    key = 'phys:Delete',
    mods = 'SHIFT',
...
-- この設定により、端末内アプリケーションへ Ctrl + S が送られなくなる。
table.insert(config.keys, {
  key = 's',
  mods = 'CTRL',
...
table.insert(config.keys, {
  key = 'j',
  mods = 'CTRL',
  -- ペイン上にラベルを表示し、入力したラベルのペインへフォーカスを移動する。キャンセルは Esc。
...
-- 値を増やすと履歴が増えるが、メモリー使用量も増える。
config.scrollback_lines = 10000
-- スクロールバー表示の有効 / 無効を設定する。
-- true の場合は、右端にスクロールバーを表示する。
config.enable_scroll_bar = true

-- フォント描画の見た目を設定する。
-- HarfBuzz の字形置換 (合字、文脈依存の置換) を無効化する。
-- `liga` は標準合字 (例: "fi" など)、`clig` は文脈依存合字、`calt` は文脈依存の字形置換を表す。
-- これらを無効化することで、記号の見た目が勝手に変わる状況を避け、等幅表示の予測可能性を優先する。
