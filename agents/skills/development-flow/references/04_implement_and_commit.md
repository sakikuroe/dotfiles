...
- 実装を進め, 検証する.
    ...
    - `git commit` の代わりに `bash ${CLAUDE_SKILL_DIR}/scripts/commit_with_signature.sh "<メッセージ>"` を使うこと. これによりコミットメッセージ末尾に `Co-authored-by: AI Agent <ai@agent.local>` trailer が自動付加される.
