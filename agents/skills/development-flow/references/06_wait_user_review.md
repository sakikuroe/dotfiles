...
PR の状態を確認し, Step 07 (修正対応) か Step 08 (マージ) のどちらに進むかを判定する.
...
- 状態に応じて次の Step を決める.
    ...
    - draft のまま正式レビューに進めない場合 → ready 化するか Step 07 で作業継続するか決める.
...
- Issue の `進捗` が PR 状態と一致しているか確認し, 不一致があれば更新する.
    - draft PR → `ドラフトレビュー中`.
...
- `reviewDecision` だけでなくコメント内容も確認すること. 具体的な修正依頼があれば Step 07 へ進む.
- 承認済みに見えても, 競合や base 追従不足があれば Step 08 へは進めない.
- レビュー待ちの間は不要な差し替え commit / push を避けること. 必要なら Step 07 の手順で行う.
