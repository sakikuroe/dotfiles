...
- 以下のコマンドで PR の状態とレビュー全体を取得します.
    - コマンド例:
        ```bash
        gh pr view <PR番号> --json state,isDraft,reviewDecision,mergeable,mergeStateStatus,statusCheckRollup,reviews
        ```
    - `reviews` から `state: COMMENTED` の件数と本文を確認します. コメント本文に具体的な指摘や要求が含まれていれば Step 07 へ進みます.
- インライン review comment は `gh pr view --json reviews` には含まれないため, 別途取得します.
    - インライン comment があれば Step 07 へ進みます.
    - コマンド例:
        ```bash
        # path, line, body, id を含む一覧
        gh api repos/<owner>/<repo>/pulls/<PR番号>/comments \
            --jq '.[] | {id, path, line, original_line, original_commit_id, in_reply_to_id, body}'
        ```
    - `in_reply_to_id` が null のものが起点コメント. 値が入っているものは既存スレッドへの返信.
- 状態に応じて次の Step を決めます.
    - `MERGED` の場合 → Step 08.
