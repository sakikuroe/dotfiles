...
- [scripts/add_progress_comment.sh](./scripts/add_progress_comment.sh): Issue に進捗コメントを追加投稿する. 状態変化のたびに呼ぶ (Step 04, 05, 07, 08).
...
- [scripts/reply_inline.sh](./scripts/reply_inline.sh): インライン review comment に返答する. 本文末尾に署名を自動付加する. コミットハッシュを指定するとそのコミットの URL を署名直前に挿入する. ハッシュの代わりに `-` を渡すとコミット URL の挿入をスキップする. ハッシュ指定時は該当指摘に対応したコミットを明示的に指定すること (Step 07).
...
- [scripts/commit_with_signature.sh](./scripts/commit_with_signature.sh): コミットメッセージ末尾に `Co-authored-by: AI Agent <ai@agent.local>` trailer を追加して `git commit` する (Step 04).
- [scripts/create_pr.sh](./scripts/create_pr.sh): 本文末尾に署名を自動付加して PR を作成する (Step 05).
