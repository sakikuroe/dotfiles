...
- [scripts/add_progress_comment.sh](./scripts/add_progress_comment.sh): Issue に進捗コメントを追加投稿する. 状態変化のたびに呼ぶ (Step 04, 05, 06, 08, 09).
...
- [scripts/commit_with_signature.sh](./scripts/commit_with_signature.sh): コミットメッセージ末尾に `Co-authored-by: AI Agent <ai@agent.local>` trailer を追加して `git commit` する (Step 05).
